...
public struct Part1 {
    ...
    public func calculate(_ input: String) -> Int {
        ...
        for line in input.split(separator: /\n/) {
            let lineWithoutCardNumber = line.replacing(/Card\s\d+:/, with: "")
            let split = lineWithoutCardNumber.split(separator: /\|/)
            let winningNumbers = split[0].split(separator: " ")
            let numbersToCheck = split[1].split(separator: " ")

            let actualWinningNumbers = winningNumbers
                .filter { numbersToCheck.contains($0) }

            if actualWinningNumbers.isEmpty { continue }

            let power = actualWinningNumbers.count - 1
